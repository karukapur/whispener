...
4. While listening, Pause stops microphone capture and freezes the active-listening timer without closing the current session. Resume continues the same transcript, English context, session, and adaptive summary phase.
5. For English context, configure at least one remote key and enable summaries. The default is Groq GPT-OSS 20B when `GROQ_API_KEY` is present in a debug build; Settings also offers `openrouter/free` and five compatible low-latency OpenRouter models when an OpenRouter key is configured.
...
- Pause keeps the current session open while stopping microphone capture, releasing audio focus, freezing elapsed active-listening time, and blocking new Groq/OpenRouter requests. Already queued transcription or an already issued remote request may finish; finalized Chinese that was not summarized remains pending until Resume.
...
- Runtime traces include `paused`, `pause_recording_requested`, `resume_recording_requested`, and `summary_attempt_skipped reason=listening_paused` when a summary tick or fallback is blocked by Pause.
...
- Groq uses the fixed `openai/gpt-oss-20b` model with low reasoning effort, strict non-streaming structured output, smaller prompt payloads, and a local rolling token-budget guard to avoid token-per-minute rejections. Remote summaries currently use a shared adaptive cadence trial across the remote model suite: 5 seconds for the first minute, 8 seconds for the second minute, and 10 seconds afterward. The schedule is based on Groq cadence experiments and should be revalidated before treating it as OpenRouter-specific evidence.
